...
io.on( "connection", function( socket ) {
    ...
    socket.on("car-navigation", function(value) {
        ...
        var cmd = "/home/debian/self_driving_car/spi/Write " + speedLeft + "," + speedRight + "#";
    ...
    socket.on("car-ocv", function(value) {
        ...
        if(stat == "start") {
            var cmd = "/home/debian/self_driving_car/open_cv/BallTracker " + device + " " + color;
            console.log("Executing: " + cmd);
            exec(cmd,{silent:true});
        } else if(stat == "stop") {
            var cmd = "kill -9 $(ps -edf | grep 'BallTracker " + device + " " + color + "' | grep -v grep | awk '{print$2}');";
			cmd += " /home/debian/self_driving_car/spi/Write 0,0#";
            console.log("Executing: " + cmd);
            exec(cmd,{silent:true});
        }
